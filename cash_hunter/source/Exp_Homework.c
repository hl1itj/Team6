...
Exp_4_Homework_A(void)
{
	...
	while (1) {
			...
			if ((key < 16) && (direction == TO_LEFT)){
				//all_key = all_key | (position + key);
				//writeb_virtual_io(SEG7LED, all_key);
				//position = position - 0x0010 ; // 왼쪽으로
                
                               all_key[8 - count%8] = key;
                               if(count == 8)
                                  count = 0;
                               else
                                  count++;
                                for(i = 0; i < 8;i++){
                                     ...
                                     writeb_virtual_io(SEG7LED, position + all_key[i]);
                                }

			} else if((key < 16) && (direction == TO_RIGHT)){
				//all_key = all_key | (position + key);
				//writeb_virtual_io(SEG7LED, all_key);
				//position = position + 0x0010 ; // 오른쪽으로
			       all_key[8 - count%8] = key;
                               if(count == 8)
                                  count = 0;
                               else
